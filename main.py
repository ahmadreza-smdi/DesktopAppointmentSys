...
class patient:
    def __init__(self,fname,lname,id_number,national_id
			...
			cur.execute(("INSERT INTO patient ( fname, lname,id_number ,national_id,phone_number,address ,plate_number,post_code,special_disease,blood_type)
                                        VALUES ('%s','%s','%s' ,'%s','%s','%s','%s','%s','%s','%s')")
				                                %(self.fname, self.lname,self.id_number ,self.national_id,self.phone_number,self.address ,self.plate_number,self.post_code,self.special_disease,self.blood_type))
...
def del_patient():
    ...
    cur.execute(("DELETE FROM patient WHERE id_number = '%s'")%(del_id))

def update_patient():
    up_id = input("Enter the id that you like to update:")
    up_field =input ("What do you want to update ?").lower()
    a = []
    a = cur.execute(("SELECT '%s' FROM patient WHERE id_number = '%s'")%(del_id,up_field))
    print("The last value of that was {}".format(a))
    b = input("enter the new value:")
    cur.execute("UPDATE patient SET '%s' = '%s' WHERE id_number = '%s' ")%(up_field,b,up_id)
